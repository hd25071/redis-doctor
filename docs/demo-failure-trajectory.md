...
summary: 副本 demo-1 / demo-2 日志出现 Name or service not known，主从同步持续失败。
labels: alertname=RedisReplicaCannotResolveMaster, severity=critical

--- triage ---
对象: ns=demo instance=demo pods=['demo-1', 'demo-2'] 症状=replication
...
--- 工具调用轨迹 ---
  c001-dde18d k8s_get_pods({"label_selector": ""}) -> signals=[] 
      | demo-0     phase=Running   ready=True restarts=0 age=5400s
      | demo-1     phase=Running   ready=True restarts=0 age=5400s
      | demo-2     phase=Running   ready=True restarts=0 age=5400s
  c002-01ec28 k8s_events({"involved_object": ""}) -> signals=[] 
  ...
  c003-1c5a34 redis_info({"pod": "demo-0", "section": "all"}) -> signals=['master_role_confirmed'] 
      | uptime_in_seconds: 5400
      | connected_clients: 3
      | maxclients: 10000
      | blocked_clients: 0
  c004-e6a43a redis_info({"pod": "demo-1", "section": "replication"}) -> signals=['replica_link_down', 'replica_role_confirmed'] 
      | role: replica
      | master_host: demo-0.demo-headless.demo.svc.cluster.local
      | master_link_status: down
      | master_last_io_seconds_ago: 1
  c005-6fced4 prom_query({"template": "redis_master_link_up"}) -> signals=['replica_link_down'] 
      | redis_master_link_up{pod="demo-1"} 0
      | redis_master_link_up{pod="demo-2"} 0
  c006-5cac55 redis_config_get({"pod": "demo-0", "param": "repl-backlog-size"}) -> signals=['repl_backlog_small'] 
...
证据:
  - c004-e6a43a redis_info [replica_link_down] master_link_status: down
  - c005-6fced4 prom_query [replica_link_down] redis_master_link_up{pod="demo-1"} 0
排除项:
...
--- 预算与安全 ---
步数/工具调用/耗时: {'max_steps': 12, 'max_tool_calls': 30, 'max_seconds': 180.0, 'steps': 5, 'tool_calls': 6, 'elapsed_seconds': 0.004, 'exhausted': False}
